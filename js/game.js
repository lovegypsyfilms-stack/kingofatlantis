...
// v3 Safeway district 3072x1536: walkable streets, sidewalks, lot (measured from the plates).
const STREET = {
  w: 3072, h: 1536,
  walk: [
    [0, 36, 3072, 166], [0, 1000, 3072, 176], [362, 0, 170, 1536], [2436, 0, 168, 1536],
    [1480, 1142, 120, 394], [1520, 0, 110, 90], [940, 580, 1010, 420], [1060, 530, 730, 70], [860, 170, 80, 830], [1950, 560, 60, 440],
  ],
  block: [[1040, 880, 240, 120], [1390, 880, 460, 120]],
  nodes: [[404, 1330], [450, 1330], [450, 1090], [450, 120], [900, 120], [900, 590], [900, 1090], [1335, 1090], [1335, 700], [1430, 600], [1430, 560],
    [1540, 1090], [1540, 1500], [1575, 120], [1575, 40], [1980, 1090], [1980, 600], [2520, 120], [2520, 1090], [2520, 1500], [450, 1500], [2520, 40], [450, 40], [3000, 120], [3000, 1090], [100, 120], [100, 1090]],
  home: [372, 1330], homeSpawn: [404, 1330], safeway: [1430, 556], safewaySpawn: [1430, 628],
  pedLanes: [
    { a: [0, 48], b: [3072, 48] }, { a: [0, 190], b: [3072, 190] }, { a: [0, 1014], b: [3072, 1014] }, { a: [0, 1162], b: [3072, 1162] },
    { a: [378, 0], b: [378, 1536] }, { a: [518, 0], b: [518, 1536] }, { a: [2450, 0], b: [2450, 1536] }, { a: [2590, 0], b: [2590, 1536] },
    { a: [1070, 566], b: [1780, 566] },
  ],
  carLanes: [
    { a: [-150, 136], b: [3222, 136] }, { a: [3222, 100], b: [-150, 100] }, { a: [-150, 1112], b: [3222, 1112] }, { a: [3222, 1062], b: [-150, 1062] },
    { a: [470, -150], b: [470, 1686] }, { a: [425, 1686], b: [425, -150] }, { a: [2546, -150], b: [2546, 1686] }, { a: [2496, 1686], b: [2496, -150] },
  ],
};

/* ------------------------------------------------------------------ state machine definition */
const LOOP = ['NIGHT_CLOSE', 'ASTRAL_REVEAL', 'LESSER_ENTITIES', 'DEMON_BATTLE', 'DAWN', 'ROOM_MORNING', 'BACKYARD_MORNING',
  'STREET_OUTBOUND', 'SAFEWAY', 'STREET_RETURN', 'BACKYARD_EVENING', 'ROOM_EATING', 'BED'];
...
const MODE_OF = {
  ...
  BACKYARD_MORNING: 'yard', STREET_OUTBOUND: 'street', SAFEWAY: 'street', STREET_RETURN: 'street', BACKYARD_EVENING: 'yard', ROOM_EATING: 'room', BED: 'room',
};
...
const Game = {
  ...
  loadSave(s) { this.stats = { ...s.stats }; this.inv = { ...s.inv }; this.day = s.day; this.go(s.state, { fade: 0.8 }); },
  ...
  complete() { this.go(NEXT[this.state], S[this.state].exitFade || { fade: 0.5 }); },
  ...
  planPath(fx, fy, tx, ty) {
    const can = this.mode === 'yard' ? yardCanStand : this.mode === 'street' ? streetCanStand : benRoomCanStand;
    const nodes = this.mode === 'yard' ? YARD.nodes : this.mode === 'street' ? STREET.nodes : null;
  ...
  drawStreet() {
    const ctx = this.ctx; const m = Assets.meta.map_safeway;
    ctx.fillStyle = '#1f2a18'; ctx.fillRect(-2000, -2000, 8000, 6000);
    if (!m) return;
    // exact 512x512 plates at (col-1)*512,(row-1)*512 — no resize, spacing, overlap or perspective
    for (const t of m.tiles) { const im = Assets.img[`tile_safeway_${t.row}_${t.col}`]; if (im) ctx.drawImage(im, (t.col - 1) * 512, (t.row - 1) * 512, 512, 512); }
    if (this.debug) { ctx.strokeStyle = 'rgba(0,255,120,0.5)'; ctx.lineWidth = 3; STREET.walk.forEach(r => ctx.strokeRect(r[0], r[1], r[2], r[3]));
      ctx.strokeStyle = 'rgba(255,60,60,0.6)'; STREET.block.forEach(r => ctx.strokeRect(r[0], r[1], r[2], r[3])); }
  },
  drawFx(ctx) {
    this.fx.each(f => {
  ...
  drawTitle(dt) {
    ...
    if (sp && sp.complete && sp.naturalWidth) {
      ...
      // landscape: cover; portrait: frame the painted title (x 820..1580) and letterbox above/below
      const k = portrait ? this.W / 780 : Math.max(this.W / sp.naturalWidth, this.H / sp.naturalHeight);
      const ox = portrait ? -810 * k : clamp(this.W / 2 - sp.naturalWidth * k * 0.5, this.W - sp.naturalWidth * k, 0);
    ...
    // buttons sit under the painted title (image x≈1195, y≈690) when it's on screen, else bottom-centre
    let cx = this.W / 2, top = this.H * 0.64;
    if (map) { const [mx, my] = map(1195, 670); cx = clamp(mx, 150, this.W - 150); top = Math.min(my, this.H - (Save.read() ? 120 : 70)); }
    if (!this.ready) {
    ...
    if (save) btn(`Continue — Day ${save.day}, ${save.state === 'NIGHT_CLOSE' ? 'night' : 'morning'}`, () => this.loadSave(save));
    btn(save ? 'Start over (Day 1 night)' : 'Begin the night', () => { Save.clear(); this.newGame(); });
...
S.LESSER_ENTITIES = {
  ...
  update(dt, live) {
    ...
    this.camTo(ROOM.battle[0], ROOM.battle[1] - 30, Math.min(this.W / (2 * 1400), this.H / (2 * 1400 * 0.62 + 380)), 1.2, dt);
    const e = s.cur;
    ...
    const px = -dy / d, py = dx / d, weave = Math.sin(e.t * 2.2) * e.type.weave;
...
S.DEMON_BATTLE = {
  ...
  update(dt, live) {
    ...
    this.camTo(ROOM.battle[0], ROOM.battle[1] - 30, Math.min(this.W / (2 * (orbitR + 300)), this.H / (2 * (orbitR * CFG.demonOrbitRYRatio + 330))), 0.8, dt);
    if (s.victoryT >= 0) {
      s.victoryT += dt;
    ...
    const speed = CFG.demonAngSpeed[Math.min(d.hits, 4)];
    switch (d.mode) {
      case 'enter':
      ...
      case 'lungeWarn':
        ...
        break;
      case 'lunge':
        d.lean = Math.min(1, d.lean + dt / 1.8 * (0.4 + d.t)); // accelerating final inward lunge
...
S.DAWN = {
  ...
  update(dt) {
    ...
    this.camTo(ROOM.w / 2, ROOM.h / 2, this.roomFitZoom() * 1.08, 1.2, dt);
...
S.ROOM_MORNING = {
  enter() {
    ...
    Object.assign(this.cam, { x: ROOM.w / 2, y: ROOM.h / 2, z: this.roomFitZoom() * 1.08 });
  ...
  update(dt, live) {
    ...
    this.camTo(ROOM.w / 2, ROOM.h / 2, this.roomFitZoom() * 1.08, 2, dt);
...
function yardEnter(evening) {
  ...
  this.sub = { evening, pig: { x: YARD.hutch[0], y: YARD.hutch[1], state: 'idle', t: 0, facing: 1, eatFrame: 0 }, noticeT: 0.9, fed: false, feeding: null, walkIn: evening ? 0.8 : 0.6 };
...
function yardUpdate(dt, live) {
  ...
  if (s.nearExit && Input.action()) { if (allowed) { Audio.sfx(s.evening ? 'door' : 'bag'); this.complete(); } else this.say('The guinea pig is waiting for a carrot'); }
}
...
const streetCanStand = (x, y) => x > 4 && y > 4 && x < STREET.w - 4 && y < STREET.h - 4 &&
  STREET.walk.some(r => x >= r[0] && x <= r[0] + r[2] && y >= r[1] && y <= r[1] + r[3]) &&
  !STREET.block.some(r => x >= r[0] && x <= r[0] + r[2] && y >= r[1] && y <= r[1] + r[3]);
function streetZoom() { return clamp(Math.max(this.H / 760, this.W / 1350), 0.45, 2.2); }
function streetEnter(ret) {
  const b = this.ben; const sp = ret ? STREET.safewaySpawn : STREET.homeSpawn;
  Object.assign(b, { x: sp[0], y: sp[1], asleep: false, h: CFG.benStreetHeight, dir: ret ? 4 : 2, pose: 'idle', moving: false });
  const z = streetZoom.call(this); Object.assign(this.cam, { x: b.x, y: b.y, z });
  this.sub = { ret, rng: this.rngFor(ret ? 'return' : 'outbound'), slowT: 0, invuln: 0, carT: 1.5, seekers: 0, ingress: 0, carHits: 0, visited: new Set() };
  this.peds.clear(); this.cars.clear(); this.waves.clear(); b.pose = 'idle';
  this.say(Input.touchMode ? 'Tap a person or press FIRE to push them back' : 'Space / F or click a person to push them back', 3.5);
  const nPeds = ret ? 12 : 10; for (let i = 0; i < nPeds; i++) spawnPed.call(this, true);
  this.objective = ret ? 'GET HOME' : 'GET FOOD FROM SAFEWAY';
  Audio.ambience({ street: 0.35, morning: ret ? 0 : 0.2, evening: ret ? 0.2 : 0 }, 1.5);
}
...
const LANE_PAIR = [1, 0, 3, 2, 5, 4, 7, 6, 2]; // opposite sidewalk of the same street
...
function spawnPed(initial) {
  ...
  let li;
  do { li = Math.floor(r() * STREET.pedLanes.length); lane = STREET.pedLanes[li]; t = r(); x = lerp(lane.a[0], lane.b[0], t); y = lerp(lane.a[1], lane.b[1], t); tries++; }
...
function spawnCar() {
  const s = this.sub, r = s.rng; const lane = STREET.carLanes[Math.floor(r() * STREET.carLanes.length)];
...
function streetUpdate(dt, live) {
  ...
  const z = streetZoom.call(this); const vw = this.W / z, vh = this.H / z;
  const cx = vw >= STREET.w ? STREET.w / 2 : clamp(b.x, vw / 2, STREET.w - vw / 2), cy = vh >= STREET.h ? STREET.h / 2 : clamp(b.y, vh / 2, STREET.h - vh / 2);
  ...
  if (live && !s.locked) {
    this.moveBen(dt, CFG.benStreetSpeed * (s.slowT > 0 ? 0.6 : 1), streetCanStand);
...
function streetFireAt(tx, ty) {
  ...
  if (st.astral < CFG.streetShotCost) { Audio.sfx('empty'); this.say('No astral energy left'); return false; }
  ...
  st.astral -= CFG.streetShotCost; s.fireCd = CFG.streetCooldown; s.shots = (s.shots || 0) + 1;
...
function pedReaction(p, dt) {
  ...
  if (p.mode === 'knock') {
    ...
    if (p.kt <= 0) {
      if (p.after === 'cross') {
        const li = LANE_PAIR[p.li], L = STREET.pedLanes[li]; const horiz = L.a[1] === L.b[1];
        p.cross = horiz ? { x: clamp(p.x, 20, STREET.w - 20), y: L.a[1] } : { x: L.a[0], y: clamp(p.y, 20, STREET.h - 20) };
  ...
  if (p.mode === 'cross') { // cross to the other sidewalk, then keep walking there
    ...
    if (d < 6) {
      p.li = p.newLi; p.lane = STREET.pedLanes[p.li]; const L = p.lane;
...
function streetDraw(ctx) {
  ...
  // target markers
  const tgt = s.ret ? STREET.home : STREET.safeway; const pulse = 0.5 + 0.5 * Math.sin(this.time * 4);
...
S.STREET_OUTBOUND = {
  enter() { streetEnter.call(this, false); },
  update(dt, live) {
...
S.SAFEWAY = { // entering is a trigger: fade, purchase, respawn outside with the bag
  ...
  draw(ctx) { const b = this.ben; drawBen(ctx, b.x, b.y, b.h, b.dir, 0, { bag: true }); },
  exitFade: { fade: 0 },
};
S.STREET_RETURN = {
  enter() { streetEnter.call(this, true); },
  update(dt, live) {
    streetUpdate.call(this, dt, live);
    if (live && dist(this.ben.x, this.ben.y, STREET.home[0], STREET.home[1]) < 48) this.complete();
  },
...
const UI = {
  ...
  fireButton() { const G = Game; return G.mode === 'street' && G.state !== 'SAFEWAY' ? { x: G.W - 150, y: G.H - 60, r: 32 } : null; },
  ...
  draw(ctx) {
    ...
    const quiet = ['NIGHT_CLOSE', 'ASTRAL_REVEAL', 'DAWN'].includes(G.state);
    ctx.save();
    if (!quiet) {
      ...
      if (!battle) {
        ...
        ctx.fillText(`$${st.money}  ·  🥕 ${G.inv.carrots}${G.inv.carryingBag ? '  ·  🛍' : ''}  ·  Day ${G.day}`, x, y + 80);
      }
      ctx.globalAlpha = 1;
    ...
    if (G.mode === 'street') this.streetHud(ctx);
  ...
  streetHud(ctx) {
    const G = Game, b = G.ben; const ret = G.state === 'STREET_RETURN'; const tgt = ret ? STREET.home : STREET.safeway;
    // minimap
    const mw = Math.min(180, G.W * 0.3), k = mw / STREET.w, mh = STREET.h * k, mx = G.W - mw - 12, my = 12;
    ctx.fillStyle = 'rgba(10,14,10,0.6)'; ctx.fillRect(mx - 3, my - 3, mw + 6, mh + 6);
    ctx.fillStyle = 'rgba(200,200,190,0.55)'; STREET.walk.forEach(r => ctx.fillRect(mx + r[0] * k, my + r[1] * k, Math.max(1, r[2] * k), Math.max(1, r[3] * k)));
  ...
  debug(ctx) {
    ...
    const lines = [
      ...
      G.mode === 'street' ? `map tile r${Math.floor(b.y / 512) + 1}-c${Math.floor(b.x / 512) + 1}  ben (${b.x | 0},${b.y | 0}) dir ${DIRS[b.dir]}  peds ${G.peds.count()} cars ${G.cars.count()}` : `ben (${b.x | 0},${b.y | 0}) dir ${DIRS[b.dir]} pose ${b.pose}${b.asleep ? ' asleep' : ''}`,
...
/* test / tooling hook */
window.KOA = {
  get state() { return Game.state; }, game: Game, S, ROOM, YARD, STREET, CFG, LOOP, NEXT, COMPLETE_WHEN, Assets,
